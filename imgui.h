...
namespace ImGui
{
    ...
    IMGUI_API void          ShowDemoWindow();                           // demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
    IMGUI_API void          ShowMetricsWindow();                        // display Dear ImGui internals: windows, draw commands, various internal state, etc.
    IMGUI_API void          ShowDebugLogWindow();                       // display a simplified log of important dear imgui events.
    IMGUI_API void          ShowIDStackToolWindow();                    // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
    ...
    // Styles
    IMGUI_API void          StyleColorsDark(ImVec4* dst = NULL);        // new, recommended style (default)
    IMGUI_API void          StyleColorsLight(ImVec4* dst = NULL);        // new, recommended style (default)
    IMGUI_API void          StyleColorsClassic(ImVec4* dst = NULL);        // new, recommended style (default)
    ...
    IMGUI_API bool          DragU32(const char* label, ImU32* v, float v_speed = 1.0f, ImU32 v_min = 0, ImU32 v_max = 0, const char* format = "%u", ImGuiSliderFlags flags = 0);  // If v_min >= v_max we have no bound
    ...
    IMGUI_API bool          SliderU32(const char* label, ImU32* v, ImU32 v_min, ImU32 v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
